...
export class CommandHandler {
	static register(): void {
		system.afterEvents.scriptEventReceive.subscribe((ev) => {
			...
			switch (cmd) {
				case "menu":
					DreamweaverUI.showMainMenu(player);
					break;
				case "wand":
					giveWand(player);
					break;
				case "pos1":
					this.setPos(player, 1, args);
					break;
				case "pos2":
					this.setPos(player, 2, args);
					break;
				case "capture":
					DreamweaverUI.showMainMenu(player).then(() => {});
					break;
				case "paste":
					DreamweaverUI.showImportForm(player);
					break;
				case "cancel":
