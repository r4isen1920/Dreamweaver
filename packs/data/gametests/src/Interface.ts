...
async function showForm<T extends FormData>(form: T, player: Player, retries = 5): Promise<Awaited<ReturnType<T["show"]>>> {
...
export class DreamweaverUI {
  ...
  static async showMainMenu(player: Player): Promise<void> {
    ...
    switch (res.selection) {
      case 0: giveWand(player); break;
      case 1: await this.captureFlow(player); break;
      case 2: await this.showImportForm(player); break;
    }
  }
  ...
  private static async captureFlow(player: Player): Promise<void> {
    ...
    if (!sel.isComplete()) {
      player.sendMessage("§cSelect two positions first using the Dreamweaver Wand.");
      return;
    }
  ...
  static async showCaptureResult(player: Player, schematic: Schematic): Promise<void> {
  ...
  static async showExportForm(player: Player, parts: string[]): Promise<void> {
    for (let i = 0; i < parts.length; i++) {
      ...
      const form = new ModalFormData()
        .title(parts.length > 1 ? `§l§dExport §r§7(${i + 1}/${parts.length})` : "§l§dExport Schematic")
  ...
  static async showImportForm(player: Player): Promise<void> {
    ...
    if (header.total > 1) {
      player.sendMessage(`§7Multi-part schematic detected (${header.total} parts). Paste each remaining part.`);
    ...
    try {
      ...
      log.info(`${player.name} imported schematic (${schematic.size.x}×${schematic.size.y}×${schematic.size.z})`);
      player.sendMessage(`§aSchematic loaded! §7(${schematic.size.x}×${schematic.size.y}×${schematic.size.z})`);
      await this.showCaptureResult(player, schematic);
    } catch (e) {
  ...
  static async showMaterialList(player: Player, schematic: Schematic): Promise<void> {
  ...
  private static async confirmPlace(player: Player, schematic: Schematic): Promise<void> {
    const total = schematic.getTotalNonAir();
  ...
  private static async confirmPreview(player: Player, schematic: Schematic): Promise<void> {
    const total = schematic.getTotalNonAir();
    const form = new MessageFormData()
      .title("§l§dPreview Placement")
      .body(`Preview §f${formatCount(total)}§r blocks at your current position?\n§7Blocks will revert after 10 seconds.`)
      .button1("Cancel")
      .button2("Preview");
    ...
    player.sendMessage(`§aPreview: ${formatCount(placed)} blocks. §7Reverting in 10s...`);
  }
}
