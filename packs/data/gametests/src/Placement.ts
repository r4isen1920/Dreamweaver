import { system, BlockPermutation, Dimension, type Vector3 } from "@minecraft/server";
...
export class PlacementSession {
  ...
  place(): Promise<number> {
    ...
    return new Promise((resolve) => {
      self.jobId = system.runJob(function* () {
        for (let i = 0; i < self.schematic.blocks.length; i++) {
          ...
          self.savedBlocks.push({ pos: Vec3.from(worldPos), permutation: block.permutation });
